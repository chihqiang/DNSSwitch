...
export function ScheduleRule({ rule, onToggle, onEdit, onDelete }: ScheduleRuleProps) {
  ...
  const conditionLabel = () => {
    switch (rule.condition.type) {
      case CondType.TIME:
        return t('schedule.condition_time', {
          start: rule.condition.timeRange.start,
          end: rule.condition.timeRange.end,
        });
      case CondType.NETWORK:
        return t('schedule.condition_network', {
          name: rule.condition.ssid || rule.condition.interfaceName || t('common.unknown'),
        });
      case CondType.CRON:
  ...
  const typeLabel = () => {
    switch (rule.condition.type) {
      case CondType.TIME:
        return t('schedule.type_time');
      case CondType.NETWORK:
        return t('schedule.type_network');
      case CondType.CRON:
        return 'Cron';
      case CondType.STARTUP:
        return t('schedule.type_startup');
      default:
        return t('schedule.type_always');
    }
  };
    ...
    <Card className={`flex flex-col gap-3 p-3 ${!rule.enabled ? 'opacity-60' : ''}`}>
      <div className="flex items-center justify-between">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
          ...
          <span
            className={`w-9 h-5 rounded-full transition-colors duration-200 relative ${rule.enabled ? 'bg-accent' : 'bg-border'}`}
          >
            <span
              className={`absolute w-4 h-4 bg-white rounded-full top-0.5 left-0.5 transition-transform duration-200 ${rule.enabled ? 'translate-x-4' : ''}`}
            />
          </span>
        </label>
      </div>
      ...
      <div className="flex gap-1.5 pt-3 border-t border-border">
        ...
        <Button variant={ButtonVariant.DANGER} size="sm" onClick={() => onDelete(rule.id, rule.name)}>
          {t('common.delete')}
        </Button>
      </div>
    </Card>
